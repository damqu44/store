  ...
  if (productData.length > 0) {
    try {
      ...
      if (response.ok) {
        ...
        console.log('Verified Cart Data:', verifiedData)
    ...
    `
        ...
        const commonDeliveryMethods = allDeliveryMethods.filter((method) =>
          verifiedData.every((product) =>
            product.DeliveryMethods.some(
              (delivery) => delivery.Id === method.Id
            )
          )
        )
        ...
        if (uniqueCommonDeliveryMethods.length > 0) {
          ...
          const defaultDeliveryMethod = createDeliveryOption(
            7,
            'Kurier DPD',
            '0'
          )
        ...
        console.error('Error verifying cart:', response.statusText)
      }
    } catch (error) {
